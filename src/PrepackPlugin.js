      ...
      compilation.plugin('optimize-chunk-assets', (chunks, callback) => {
        for (const chunk of chunks) {
          ...
          for (const file of files) {
            ...
            if (
              !ModuleFilenameHelpers.matchObject(matchObjectConfiguration, file)
            ) {
            ...
            const code = isEntryChunk(chunk)
              ? asset.source()
              : `${nonEntryChunkPreamble}\n ${asset.source()}`;

            const prepackedCode = prepackSources([{ fileContents: code }], {
